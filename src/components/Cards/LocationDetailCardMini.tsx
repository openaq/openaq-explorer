...
import style from './LocationDetailCardMini.module.scss';
...
export function LocationDetailCardMini() {
  ...
  return (
    <div
      class={`${style['location-detail-card-mini']} ${
        store.locationsId
          ? ''
          : style['location-detail-card-mini--translate']
      }`}
    >
      <header class={style['location-detail-card-mini__header']}>
        <div>
        ...
        <div>
          <button class={style["close-btn"]} onClick={() => clearLocationsId()}> <img src="/svgs/close.svg" alt="close icon" /> </button>
        </div>
      </header>
      <div class={style['location-detail-card-mini__body']}>
        <section class={style['section']}>
          <span class="type-body-3">
            {location()?.results?.[0].country.name}
          </span>
        </section>
        <section class={style['section-grid']}>
        ...
        <section class={style['section-grid']}>
          ...
          <div class={style['reporting-cell']}>
      ...
      <footer class={style['location-detail-card-mini__footer']}>
        <Show when={user()} >    
