...
import style from './LocationDetailCard.module.scss';
import {
  For,
  Show,
  createEffect,
...
import {
  getLocation,
  getLocationRecentMeasurements,
  getSensorRecentMeasurements,
} from '~/client';
...
export function LocationDetailCard() {
  ...
  return (
    <div
      class={`${style['location-detail-card']} ${
        store.locationsId
          ? ''
          : style['location-detail-card--translate']
      }`}
    >
      ...
      <div class={style['location-detail-card__body']}>
        ...
        <section class={style['section-grid']}>
          ...
          <div class={style['reporting-cell']}>
            ...
            <span class="type-body-4">
              Reporting since{' '}
              {since(location()?.results?.[0].datetimeFirst.local)}
            </span>
          </div>
        </section>
        <section class={style['recent-measurements']}>
      ...
      <footer class={style['location-detail-card__footer']}>
        <Show when={user()}>
